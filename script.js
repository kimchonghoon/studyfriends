...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function switchView(viewName) {
        ...
        // Show target
        if (target) {
            // Ensure it's visible immediately (cancel any pending hides if we could, but removing class works)
            ...
            try {
                ...
                const normalizedData = jsonData.map(row => {
                    ...
                    Object.keys(row).forEach(key => {
                        ...
                        else newRow[key] = row[key]; // Keep others as is
                    });
                    return newRow;
                });

                // Filter out empty rows or rows without keywords
                ...
                if (validData.length === 0) {
                    alert(`데이터를 찾을 수 없습니다.\n\n엑셀 파일의 첫 번째 줄(헤더)을 확인해 주세요.\n필수 항목: QuestionKeyword (또는 질문/키워드)`);
                } else {
                    alert(`전체 ${jsonData.length}행 중 ${validData.length}개의 유효한 데이터를 불러왔습니다!\n(질문 키워드: 'Questions Keyword' 등도 자동 인식됨)`);
                }

                console.log('Loaded Data:', state.knowledgeBase);
            } catch (err) {
                console.error('Excel parsing error:', err);
                alert('엑셀 파일 분석 중 오류가 발생했습니다. 올바른 .xlsx 파일인지 확인해주세요.');
            }
        };
    ...
    // Force initial view render to prevent blank screen
    switchView('landing');
